...
Status at handoff: work in progress, not PR-ready. SDK composition, Rivet Effect workspace actors, guest filesystem/process RPC, workspace admission, and agentOS/E2B migration are implemented. Complete round-trip migration has not passed yet. The user explicitly requested committing and pushing this plan for the Devin handoff, superseding the earlier gitignored-only requirement. Sections below describe the original design and historical checkpoints; this handoff status supersedes their progress and authorization claims.

## Current handoff checkpoint

- The user now authorizes pushing the current work to branches in both forks. Devin on the user's Mac will finish implementation, tests, PR updates, parallel critical-only review, merge, and publication of a new OpenCode fork release. No merge or publication is part of this snapshot handoff.
- OpenCode branch: `rika-labs/opencode:rivet-sdk`, targeting `dev`. Dependency fork branch: `rika-labs/agentos:confirmed-shutdown`. Keep the agentOS runtime fixes and fork-only release machinery separable for a later focused upstream contribution.
...
- Latest live migration reached E2B after fixing symlink timestamp restoration. The test then failed because strict comparison distinguished Buffer from Uint8Array with identical bytes. Assertions now compare arrays, but have not been rerun. Test `packages/rivet/test/registry-cases/migration.ts` must prove agentOS -> E2B -> agentOS, generation fencing, modes/symlinks/binary/.git preservation, exact retries, and source cleanup.
- E2B process isolation uses a protected cgroup workload boundary. Tested Daytona configurations did not permit the required shutdown boundary; safe Daytona reverse migration remains unsupported. Do not claim lifecycle API tests prove safe workspace cutover.
- Remaining work includes interruption/crash/cleanup recovery, actual actor sleep/wake/destroy behavior, archive adversarial tests, managed search wiring, local/managed extension compatibility, SDK integration regression failures, final Oracle/security review, and reproducible Node/Bun package tests.
- Preserve typed errors across actor/provider boundaries: some actor reasons currently collapse into unavailable, and terminal promotion failures carry message text rather than structured failure details. Do not claim fully typed end-to-end error fidelity yet.
- Development dependencies use local links to upstream-named fork packages and a published 0.2.19 native sidecar. They are not a matched rebuilt release. Build/test matching JS, native, software, and protocol artifacts, publish approved @rikalabs packages, then replace all development links. Do not restore dependency patches.
- AgentOS fork Actions listed zero workflows; permission/secrets inspection returned 403. NPM_TOKEN is a GitHub Actions secret, not a local credential. Resolve Actions access without requesting or exposing its value. Fork workflow currently builds Linux x64; local Mac verification needs supported matching native binaries or a clearly documented Linux execution route.
- The OpenCode publish workflow has upstream repository guards. Merging the fork must not be assumed to publish it. Verify fork-owned package names, credentials, native artifact matrix, workflow triggers, and installation before releasing; never publish to upstream-owned destinations.
- Prior evidence (not final verification): a real 65-second actor command and Node registry suite passed; targeted Core, SDK, and Rivet checks passed before the latest edits. All observed disposable cloud resources were deleted. Fresh combined verification is required.
