...
Status at handoff: implementation and verification complete locally; release blocked on credentials. SDK composition, Rivet Effect workspace actors, guest filesystem/process RPC, workspace admission, and agentOS ↔ E2B round-trip migration are implemented and verified on native macOS arm64 (Bun and bundled Node) plus live E2B and Daytona runs. No push, PR, merge, or publication has occurred: GitHub authentication is invalid (gh token dead, no keychain credential, no env token) and npm has no login. Sections below describe the original design and historical checkpoints; this handoff status supersedes their progress and authorization claims.

## Current handoff checkpoint

- Authorization already granted: push both fork branches, create/update fork PRs, merge after satisfactory review and testing, publish the OpenCode fork release, publish required `@rikalabs` agentOS fork packages, and submit a focused upstream agentOS runtime-fix PR after checking upstream issues/PRs. None of that has happened yet — it is all blocked on restored GitHub and npm credentials.
...
- Known limitations (honest, unfixed):
  ...
  - Development dependencies still use `link:` to upstream-named fork packages. Replacing them with published `@rikalabs` versions is mechanical and waits on npm publish; do not commit `file:` or unpublished version specs.
...
- Remaining before merge: restore GitHub auth (`gh auth login` or a token), push both branches, open the two fork PRs (opencode `rivet-sdk`→`dev`; agentos `confirmed-shutdown`), review, publish `@rikalabs` packages through the fork workflow (linux-x64-gnu matrix), swap `link:` deps for published versions, re-run the full suite, merge, then release the OpenCode fork.
