...
- Released: `Rika-Labs/opencode#1` merged to `dev` (`bc52d92b1`) at dev-parity-plus CI (all attributable checks green; e2e failures match the pre-existing `dev` flake baseline). `@rikalabs/opencode@1.18.30-rika.1` is published on npm under `latest` with all seven `@rikalabs/opencode-<platform>` binary packages (publish-npm.yml run `35015960787`); a clean `npm install` runs `opencode --version` → `1.18.30-rika.1`. Large binary tarballs take several minutes of npm processing before their versions index — expect `ETARGET` noise if installing in the first minutes after publish.
