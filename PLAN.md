...
Status at handoff: implementation and verification complete; release in flight. SDK composition, Rivet Effect workspace actors, guest filesystem/process RPC, workspace admission, and agentOS ↔ E2B round-trip migration are implemented and verified on native macOS arm64 (Bun and bundled Node) plus live E2B and Daytona runs. GitHub auth was restored via device flow (`repo` + `workflow` scopes); both branches are pushed and the agentOS fork PRs are merged. `@rikalabs` publication is running through `Rika-Labs/agentos` `publish.yaml` (npm token lives in Actions secrets). Sections below describe the original design and historical checkpoints; this handoff status supersedes their progress and authorization claims.

## Current handoff checkpoint

- Shipped so far: `rivet-sdk` pushed; `Rika-Labs/opencode#1` open (`rivet-sdk`→`dev`, CI red only because `link:` deps are unresolvable on runners until publish lands). `confirmed-shutdown` merged via `Rika-Labs/agentos#1`; `Rika-Labs/agentos#2` dropped the `codex-wasm` publish job, then `Rika-Labs/agentos#3` restored it with the root-cause fix (dropping tokio's `[patch.crates-io]` line invalidates the shipped lockfile entry, so `cargo vendor` floated tokio to 1.53.1 while `std-patches/crates/tokio` targets 1.52.3 — fixed by `generate-lockfile` + `update -p tokio --precise 1.52.3` before vendoring); `Rika-Labs/agentos#4` then added the actual root-cause fix — `make codex` now depends on `patch-std` because the tokio wasi companion needs `From<ChildStd*> for OwnedFd` from `std-patches/std/os/wasi/process.rs`, which only `patch-std` installs into the toolchain rust-src (upstream's self-hosted runner had it pre-applied; cold `ubuntu-latest` did not). The upstream PR `rivet-dev/agentos#1977` was closed per user direction — all PRs target the Rika-Labs forks only; the chown fix ships via `Rika-Labs/agentos`. `@rikalabs` `0.2.19-rika.1` publish re-running under dist-tag `next` (run `34999783520`).
...
- Known limitations (honest, unfixed):
  ...
  - Development dependencies previously used `link:` to upstream-named fork packages resolved via bun's global link registry — this is why the opencode PR CI cannot install. Swap staged on `rivet-sdk`: all `@rivet-dev/agentos-*` import sites renamed to `@rikalabs/*`, deps pinned to `0.2.19-rika.1`, and `@rikalabs/*` added to `minimumReleaseAgeExcludes` (user approved; the 3-day floor would refuse fresh packages). `bun install` + lockfile regen pending the publish landing.
...
- Remaining before merge: wait for the `publish.yaml` run to land `@rikalabs@0.2.19-rika.1` on npm (linux-x64-gnu sidecar matrix only — darwin resolves via the published platform-optional layout), apply the dep swap + `@rikalabs/*` release-age exclusion, re-run the full suite on published packages, confirm the opencode PR goes green, merge, then release the OpenCode fork.
