...
- Shipped so far: `rivet-sdk` pushed; `Rika-Labs/opencode#1` open (`rivet-sdk`→`dev`, CI red only because `link:` deps are unresolvable on runners until publish lands). `confirmed-shutdown` merged via `Rika-Labs/agentos#1`; `Rika-Labs/agentos#2` dropped the `codex-wasm` publish job (build-time `cargo vendor` floats dep versions — a post-Sept-9 tokio release broke the vendored wasi patch; codex software is upstream-published and outside the `@rikalabs` set). Upstream fix submitted as `rivet-dev/agentos#1977` (chown no-op, references `#1872`). `@rikalabs` `0.2.19-rika.1` publish running under dist-tag `next` (run `34990346552`).
...
- Known limitations (honest, unfixed):
  ...
  - Development dependencies still use `link:` to upstream-named fork packages resolved via bun's global link registry — this is why the opencode PR CI cannot install. Swap in progress: rename the 7 `@rivet-dev/agentos-*` import sites to `@rikalabs/*` and pin `0.2.19-rika.1`. bunfig `minimumReleaseAge` (3d) would refuse fresh packages; user approved adding `@rikalabs/*` to `minimumReleaseAgeExcludes`.
