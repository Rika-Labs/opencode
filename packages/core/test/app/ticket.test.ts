...
const itExpiring = testEffect(
  LayerNode.compile(AppTicket.node, {
    replacements: [AppTicket.node.replace(Layer.effect(AppTicket.Service, AppTicket.make(5, 5)))],
  }),
)
  ...
  it.live("round-trips the workspace scope through a session", () =>
    Effect.gen(function* () {
      ...
      const scope = {
        ...
        directory: "/tmp/a",
        workspaceID: Workspace.ID.ascending(),
      }
