...
import { AppHost } from "@opencode/core/app-host"
    ...
    Effect.acquireRelease(
      ...
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          ...
          const first = yield* host.publish(info(directory), AbsolutePath.make(path.join(tmp.path, "web")))
          const second = yield* host.publish(info(directory), AbsolutePath.make(path.join(tmp.path, "web")))
    ...
    Effect.acquireRelease(
      ...
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          const directory = AbsolutePath.make(tmp.path)
          const host = yield* AppHost.Service
          const release = yield* host.publish(info(directory), AbsolutePath.make(path.join(tmp.path, "web")))
          ...
          const next = yield* host.publish(info(directory), AbsolutePath.make(path.join(tmp.path, "web")))
