...
test("E2B cgroup fences detached work across stop and pause", { timeout: 300_000, skip: enabled ? false : "set E2B_LIVE=1" }, async () => {
  ...
  try {
    ...
    assert.equal(privileges.exitCode, 0, privileges.stderr.toString())
    assert.match(privileges.stdout.toString(), /NoNewPrivs:\s+1/)
    assert.match(privileges.stdout.toString(), /CapInh:\s+0{16}/)
    assert.match(privileges.stdout.toString(), /CapPrm:\s+0{16}/)
    assert.match(privileges.stdout.toString(), /CapEff:\s+0{16}/)
    assert.match(privileges.stdout.toString(), /CapAmb:\s+0{16}/)
    ...
    assert.equal(writer.exitCode, 0, writer.stderr.toString())
